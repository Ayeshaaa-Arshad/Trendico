...
class HomeViewTest(TestCase):
    def setUp(self):
        ...
        self.product1 = Product.objects.create(name='Product 1', price=10.00, initial_quantity=10, category=self.category)
        self.product2 = Product.objects.create(name='Product 2', price=15.00, initial_quantity=5, category=self.category)
...
class RemoveFromCartViewTest(TestCase):
    def setUp(self):
        ...
        self.cart_item = CartItem.objects.create(cart=self.cart, product_id=1, quantity=2)

    def test_remove_cart_item(self):
        self.client.login(username='testuser', password='testpassword')
        response = self.client.post('/remove_from_cart/{}/'.format(self.cart_item.id))
        self.assertEqual(response.status_code, 200)
...
class RemoveFromWishlistViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.user = User.objects.create_user(username='testuser', password='testpassword')
        self.category = ProductCategory.objects.create(name='Test Category')  # Create a category instance
        self.product = Product.objects.create(name='Test Product', price=10.00, initial_quantity=10, category=self.category)
...
class ProductViewTest(TestCase):
    def setUp(self):
        self.client = Client()
        self.category = ProductCategory.objects.create(name='Test Category')  # Create a category instance
        self.product = Product.objects.create(name='Test Product', price=10.00, initial_quantity=10, category=self.category)  # Provide valid category value
